...
def build_runtime_brief(decision: RouteDecision, message: str, job_dir: Path,
    ...
    lines += [
        ...
        "- **单次写入硬规则（2026-09-22 实测校准）**：一次 `fs.write` / `fs.append` 的 "
        "`content` **≤ 2000 字**；长文（>2000 字）必须用 `fs.append` **分多次追加**"
        "（每次 1500–2000 字）。**禁止一次写整篇** —— 超过输出上限会被截断，"
        "触发一次“升额重试”把整段上下文再发一遍，单轮成本直接翻倍"
        "（实测两单分别 358,518 / 275,277 tok 都撞在这一点上）。",
