...
8. 证据纪律：外部资料（writeup/博客/搜索命中的原题答案）只能当思路线索，永远不是证据；题目常被平台改编（flag 被替换/截断/重注入），外部答案与本地附件不一致是常态。
9. 汇报任何“找到的 flag/答案/漏洞/文件内容”前，必须与本地可复现证据逐字核验（本机附件、服务端实际交互输出、本地执行结果）；无法本地复现的声明必须显式标注“外部来源、未本地验证”，禁止当作事实上报。
10. 本地实测与外部资料冲突时，以本地实测为准，并在结果里报告该差异。
11. 提交/上报被判定错误时，先核查环境状态（目标容器/任务/服务是否就绪、是否已结束、提交时机），再归因于内容错误；不得仅凭一次拒绝就断定“平台答案不符”。
"""
